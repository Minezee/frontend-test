...
const Select = ({ value, onSelect, options, label }: SelectProps) => {
  ...
  return (
    <div className="w-full flex-1 max-w-[150px] lg:max-w-[200px] relative">
        ...
        <div className="absolute border border-gray-500 mt-2 w-full p-2 bg-white/50 backdrop-blur-xl rounded-lg z-10 hidden lg:block">
          {options?.map((option) => (
            <button
              ...
              className={`block w-full text-left text-sm p-2 rounded-lg ${value === option.value
      ...
      <div
        className={`block lg:hidden fixed ${isActive ? "top-0" : "top-full"
          } left-0 z-[999] bottom-0 w-full flex items-center justify-center transition-opacity duration-500 bg-[#262B1F]/50`}
      >
        <div
          className={`absolute flex flex-col gap-2 w-full ${isActive ? "bottom-0" : "-bottom-[999px]"
            ...
            <div className="flex flex-col gap-5">
              {options?.map((option) => (
                <button
                  key={option.value}
                  onClick={() => handleSelect(option.value)}
                  className={`flex items-center justify-between text-start ${value === option.value
                      ? "text-old-moss-green"
                      : "text-black"
                    }`}
                >
